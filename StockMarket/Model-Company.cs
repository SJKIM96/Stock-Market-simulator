...
namespace StockExchangeMarket
{
    public class Company
    {
        ...
        public Company(String symbol, String _name, double openPrice, StockMarket handledBy ,double close, double cur)
        {
            this._symbol = symbol;
            this._openPrice = openPrice;
            this._closePrice = close;
            this._currentPrice = cur;
            this.market = handledBy;
            this._name = _name;
        }
        public Company(String symbol, String _name, double openPrice, StockMarket handledBy, double close, double cur, TcpClient client, ref int CSeq, int SessionNum)
        {
            this._symbol = symbol;
            this._openPrice = openPrice;
            this._closePrice = close;
  ...
  //              SellOrders = SortedSellOrders;
        ...
        public void addBuyOrder(Double buy_price, int buy_size, DateTime dt)
        {
            if (SellOrders.Count > 0)
            {
                //              var SortedSellOrders = SellOrders.OrderBy(SellOrder => SellOrder.Price).ThenBy(SellOrder => SellOrder.TimeStamp).ToList();
                ...
                foreach (SellOrder sell_order in SellOrders)
                {
                    ...
                    if (buy_price >= sell_order.Price)
                    {
                        ...
                        else if (sell_order.Size < buy_size)
                        {
                            ...
                            // add to buyOrders for the remaining size in buyOrders list
                            int remainingSize = buy_size - sell_order.Size;
                            sale = new BuyOrder(buy_price, sell_order.Size,dt);
                            Transactions.Add(sale);
                            SellOrders.Remove(sell_order);
                    ...
                    {
                        BuyOrder buyOrder = new BuyOrder(buy_price, buy_size,dt);
                        BuyOrders.Add(buyOrder);
                        var SortedBuyOrders = BuyOrders.OrderByDescending(BuyOrder => BuyOrder.Price).ThenBy(BuyOrder => BuyOrder.TimeStamp).ToList();
                        BuyOrders = SortedBuyOrders;
                        break;
...
//                BuyOrders = SortedBuyOrders;
        ...
        public void addSellOrder(Double sale_price, int sale_size , DateTime dt)
        {
            ...
            {
                SellOrder sellOrder = new SellOrder(sale_price, sale_size,dt);
                SellOrders.Add(sellOrder);
                var SortedSellOrders = SellOrders.OrderBy(SellOrder => SellOrder.Price).ThenBy(SellOrder => SellOrder.TimeStamp).ToList();
                SellOrders = SortedSellOrders;
            }
